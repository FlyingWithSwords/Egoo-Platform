...
public class CartController {
    ...
    public JsonObject queryCartAll(@RequestParam(defaultValue = "1") Integer page,
                                    @RequestParam(defaultValue = "15") Integer limit,
                                       Cart cart){
        JsonObject object=new JsonObject();
        PageInfo<Cart> pageInfo= cartService.findCartAll(page,limit,cart);
        object.setCode(0);
        object.setMsg("ok");
    ...
    public JsonObject queryCart(Cart cart){
        JsonObject object=new JsonObject();
        List<Cart> list= cartService.findAll(cart);
    ...
    public R update(@RequestBody Cart cart){
        int num= cartService.updateData(cart);
        if(num>0){
            return R.ok();
        }else{
            return R.fail("修改失败");
    ...
    @GetMapping()
    public IPage<Cart> findListByPage(@RequestParam Integer page,
                                          @RequestParam Integer pageCount){
        return cartService.findListByPage(page, pageCount);
    }
